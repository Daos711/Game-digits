...
class GameApp:
    def __init__(self):
        ...
        self.tile_size, self.gap = 64, 3
        self.offset = (23, 23)
        self.COLORS = {1: (250, 130, 124), 2: (98, 120, 255), 3: (249, 204, 122),
        ...
        self.icon = pygame.image.load(resource_path('images/icon.png'))
        pygame.display.set_icon(self.icon)
        self.background_tile = pygame.image.load(resource_path('images/cell.JPG'))
        ...
        self.ADD_TILE_EVENT = pygame.USEREVENT + 1  # Определяем новое событие для добавления плитки
        self.tile_timer_interval = 10500  # Интервал таймера в миллисекундах (10 секунд)
    ...
    def draw_score_and_timer_window(self):
        # Определение области окна счетчика очков
        score_window_rect = pygame.Rect(self.HEIGHT + 10, 10, self.panel_width - 20, 100)
        pygame.draw.rect(self.screen, (255, 255, 255), score_window_rect)  # Белый фон окна
        pygame.draw.rect(self.screen, (0, 0, 0), score_window_rect, 2)  # Черная рамка окна

        # Рендеринг заголовка "Очки"
        score_label = self.font.render("Очки", True, (0, 0, 0))
        label_rect = score_label.get_rect(center=(score_window_rect.centerx, score_window_rect.top + 20))
        ...
        score_rect = score_text.get_rect(center=(score_window_rect.centerx, score_window_rect.top + 60))
        self.screen.blit(score_text, score_rect)
        # Определение области окна таймера
        time_window_rect = pygame.Rect(self.HEIGHT + 10, 120, self.panel_width - 20, 100)  # Под окнами с очками

        pygame.draw.rect(self.screen, (255, 255, 255), time_window_rect)  # Белый фон окна
        pygame.draw.rect(self.screen, (0, 0, 0), time_window_rect, 2)  # Черная рамка окна

        # Рендеринг заголовка "Время"
        time_label = self.font.render("Время", True, (0, 0, 0))
        label_rect = time_label.get_rect(center=(time_window_rect.centerx, time_window_rect.top + 20))
        self.screen.blit(time_label, label_rect)

        # Рендеринг текущего времени
        time_text = self.font.render(f"{self.game.current_time}", True, (0, 0, 0))
        time_rect = time_text.get_rect(center=(time_window_rect.centerx, time_window_rect.top + 60))
    ...
    def show_result_window(self):
        ...
        # Параметры окна результатов
        window_width, window_height = 400, 300
        window_surface = pygame.Surface((window_width, window_height))
        window_surface.fill((255, 255, 255))
        pygame.draw.rect(window_surface, (0, 0, 0), (0, 0, window_width, window_height), 2)
    ...
    def handle_mouse_click(self, pos):
        ...
        for arrow in self.arrows:
            if arrow.rect.collidepoint(pos):
                ...
                self.arrows.empty()
                return  # Выход после обработки стрелки
        # Затем проверяем плитки
        for tile in self.tiles:
            if tile.rect.collidepoint(pos):
                self.handle_tile_click(tile)
                break  # Предотвращаем обработку других плиток

    def handle_tile_click(self, tile):
        if self.game.selected_tile:
            if self.game.selected_tile == tile:
                # Повторное нажатие на уже выбранную плитку — ничего не делаем
                return
            elif self.game.remove_tiles(self.game.selected_tile, tile):
                self.update_display()
                self.game.selected_tile = None
                if not self.timer_running and any(self.game.board[i][j] is None for i in range(10) for j in range(10)):
                    pygame.time.set_timer(self.ADD_TILE_EVENT, self.tile_timer_interval)
                    self.timer_running = True
                return
        # Если выбирается новая плитка
        self.arrows.empty()
    ...
    def finalize_move(self, tile):
        tile.is_moving = False  # Останавливаем движение плитки
        tile.current_direction = None  # Сбрасываем текущее направление
        ...
        self.game.update_board((old_x, old_y), (new_x, new_y), tile)  # Обновляем доску
        if self.game.selected_tile == tile:
            self.game.deselect_tile()
        self.arrows.empty()
        delta_x = abs(new_x - old_x)
        delta_y = abs(new_y - old_y)
        cells_moved = delta_x + delta_y  # Так как движение либо по x, либо по y
        if cells_moved > 0:
    ...
    def update_display(self):
        self.tile_surface.fill((249, 246, 247))  # Очищаем tile_surface
        self.tiles.draw(self.tile_surface)  # Заново рисуем все плитки
    ...
    def run(self):
        running = True
        show_result = False
        prepare_to_show_result = False  # Новый флаг
        while running:
            self.draw_background()
            ...
            for event in pygame.event.get():
                if event.type == self.ADD_TILE_EVENT:
                    ...
                    self.update_display()
                    empty = any(self.game.board[i][j] is None for i in range(10) for j in range(10))
                    if not empty:
            ...
            # Проверяем флаг подготовки к завершению игры
            if self.game.prepare_to_end:
                self.game.prepare_to_end = False  # Сбрасываем флаг подготовки
                prepare_to_show_result = True  # Устанавливаем флаг ожидания перед показом результата
            # Если мы в процессе подготовки к показу результата, ждем один цикл
            elif prepare_to_show_result:
                prepare_to_show_result = False
                show_result = True  # Теперь можно показывать результат
            if show_result and not any(tile.is_moving for tile in self.tiles):
                self.update_display()  # Обновляем экран последний раз
                pygame.display.flip()
